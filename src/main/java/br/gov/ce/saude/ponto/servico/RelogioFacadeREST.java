...
//    
    ...
    @Produces({"application/json"})
    public List<Relogio> procurarPorUnidade(@PathParam("unidade") Integer unidade) {
        Query queryContrato = getEntityManager().
                createNativeQuery("select * from e_pontows.t_relogio as re where re.organograma_fk =:unidade");
        queryContrato.setParameter("unidade", unidade);
        List<Relogio> lista = queryContrato.getResultList();
