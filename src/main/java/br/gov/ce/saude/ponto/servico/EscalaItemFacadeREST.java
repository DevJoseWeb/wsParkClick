...
public class EscalaItemFacadeREST extends AbstractFacade<EscalaItem, ServiceException> {
    ...
    @GET
    @Path("listaescala/{tipoEscala}/{funcionarioContrato}/{data}")
    @Produces({"application/json"})
    public EscalaItemVO getHorario(@PathParam("tipoEscala") String tipoEscala,
            @PathParam("funcionarioContrato") BigInteger funcionarioContrato,
        ...
        try {
            ...
            Query query = getBusiness().getSesaDao().getEntityManager().
                    createQuery("FROM EscalaItem where funcionarioHorario.funcionarioContrato.id=:funcionarioContrato AND "
                            + " escala.tipo=:tipoEscala AND TO_CHAR(entrada, 'MM/YYYY')=:data "
                            + " order by entrada");

            query.setParameter("tipoEscala", tipoEscala.charAt(0));
            query.setParameter("funcionarioContrato", funcionarioContrato);
            query.setParameter("data", data);

            List<EscalaItem> escalaItens = (List<EscalaItem>) query.getResultList();

            escalaItemVO.setFuncionarioContratoFk(funcionarioContrato);
    ...
    private Object[] temEscalaSobrepoeHorario(Date entrada, Date saida, BigInteger funcionarioHorario) {
        ...
        String consulta = "SELECT                 \n"
                ...
                + ")\n"
                + "  AND \n"
                + "  t_funcionario_horario.regime_fk in (1,4,5,7) \n"
    ...
    /**
     * Valida intersecoes de escalas de usuario
     *
     * @param idFuncHorario
     * @param EscalaItem item
     */
    private void validaIntersecoesEscala(BigInteger idFuncHorario, EscalaItem item) {
        // Se o intervalo entre as datas é maior que 48 hrs
        long intervalo = (item.getSaida().getTime() - item.getEntrada().getTime()) / 1000; // segundos
        if (Math.abs(intervalo) > 60 * 60 * 48) { // Se intervalo maior que 48 hrs
            getBusiness().addErrorMessage(null, "O horario : " + sdf.format(item.getEntrada()) + " - " + sdf.format(item.getSaida())
                    + " supera 48hrs de trabalho");
            getBusiness().lancarException();
        }
        ...
        // Se existe sobreposição não grava escala
        Object[] horarioSobreposto = temSobreposicaoDeHorario(item.getEntrada(), item.getSaida(),
                idFuncHorario, item.getId());
        if (horarioSobreposto != null) {
            getBusiness().addErrorMessage(null, "A escala do usuario: " + sdf.format(item.getEntrada()) + " - " + sdf.format(item.getSaida())
                    + ",\n intercala: " + sdf.format(horarioSobreposto[1]) + " - " + sdf.format(horarioSobreposto[2]));
            getBusiness().lancarException();
        }

        // Se existe sobreposição de escala com horario diarista
        Object[] escalaSobrepoeHorario = temEscalaSobrepoeHorario(item.getEntrada(), item.getSaida(), idFuncHorario);
        if (escalaSobrepoeHorario != null) {
            getBusiness().addErrorMessage(null, "A escala do usuario: " + sdf.format(item.getEntrada()) + " - " + sdf.format(item.getSaida())
                    + ",\n intercala o horário de diarista do usuário");
            getBusiness().lancarException();
    ...
    public EscalaItemVariosDiasVO create(EscalaItemVariosDiasVO escalaItemVariosDiasVO) throws ParseException {
        ...
        // Valida as intersecoes de escala do usuario
        for (FuncionarioHorarioVO funcionario : escalaItemVariosDiasVO.getFuncionarios()) {
            for (EscalaItem dia : funcionario.getDias()) {
                ...
                validaIntersecoesEscala(funcionario.getId(), dia);
            }
        }
        ...
        for (FuncionarioHorarioVO funcionario : escalaItemVariosDiasVO.getFuncionarios()) {
            for (EscalaItem dia : funcionario.getDias()) {
                if (dia.getId() != null) {
                    ...
                    getBusiness().addErrorMessage(null, "Não foi possível remover Item!.");
                    getBusiness().lancarException();
                }
            }
        }
